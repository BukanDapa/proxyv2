...
bool events::out::pingreply(gameupdatepacket_t* packet) {
    ...
    packet->m_vec2_y = 450.f;   //move speed
    packet->m_vec_x = 640.f;     //punch range
    packet->m_vec_y = 640.f;     //build range
...
bool events::out::generictext(std::string packet) {
    ...
    if (var.get(0).m_key == "action" && var.get(0).m_value == "input") {
        ...
        else if (find_command(chat, "unpublic")) {
            gt::send_log("Set the lock to `4UnPublic");
            g_server->send(false, "action|dialog_return\ndialog_name|worldcategory\nbuttonClicked|Puzzle");
            g_server->send(false, "action|dialog_return\ndialog_name|lock_edit\ncheckbox_public|0\ncheckbox_disable_music|0\ncheckbox_disable_music_render|1\nminimum_entry_level|1");
            return true;
        }
        else if (find_command(chat, "setlevel ")) {
            std::string level = chat.substr(10);
            gt::send_log("Set The World Level To `2" + level + "");
            g_server->send(false, "action|dialog_return\ndialog_name|lock_edit\ncheckbox_public|0\ncheckbox_disable_music|0\ncheckbox_disable_music_render|1\nminimum_entry_level|" + level + "");
            return true;
        }

        else if (find_command(chat, "spamtrade ")) {
            std::string nickspam = chat.substr(11);
            gt::send_log("Sending Spam Trade To `2" + nickspam + "");
            for (int i = 0; i <= 50; ++i) {
                g_server->send(false, "action|input\n|text|/trade " + nickspam + "");
                return true;
            }
        }
        else if (find_command(chat, "spammsg ")) {
        std::string nickmsg = chat.substr(9);
        gt::send_log("Sending Spam Message To `2" + nickmsg + "");
        for (int i = 0; i <= 50; ++i) {
            g_server->send(false, "action|input\n|text|/trade " + nickmsg + "");
            return true;
        }
        ...
        else if (find_command(chat, "dupebgl")) {
        g_server->send(false, "action|dialog_return\ndialog_name|phonecall\nbuttonClicked|convertdl\n");
        gt::send_log("Sending `2Blue Gem Lock");
        return false;
        }
        ...
        else if (find_command(chat, "freeblast ")) {
        //set_default_color|`o\n\nadd_label_with_icon|big|`wThermonuclear Blast`|left|1402|\nadd_textbox|This item creates a new world!  Enter a unique name for it.|left|\nadd_text_input|thermoname|New World Name||24|\nend_dialog|usethermoblast\n
        std::string worldnyacoy = chat.substr(11);
        g_server->send(false, "action|dialog_return\ndialog_name|usethermoblast\nthermoname|" + worldnyacoy + "\n");
        gt::send_log("Sending Blasts To World " + worldnyacoy + "");
        return true;
        }
        else if (find_command(chat, "gsmexploit ")) {
        std::string MSGcoy = chat.substr(11);
        gt::send_log("`2Done Sending Global System Message  ");
        for (int i = 0; i < 60; ++i) {
            g_server->send(false, "action|input\n|text|/report CP:_`4Global System Message `w " + MSGcoy + "");
        }
        return false;
        } 
        else if (find_command(chat, "bulletin ")) { //gw agak ngantuk jadi maklum kalau lama;V
        std::string bulletin = chat.substr(10);
        gt::send_log("`2Sending Spam Board To " + bulletin + "");
        for (int i = 0; i < 40; ++i) {
            g_server->send(false, "action|dialog_return\ndialog_name|Close\nbuttonClicked|addbulletinletter_" + bulletin + "\naddbulletinletterinput|`2Raided By AstarGanz `4http://t.me/gscrip0rt");
        }
        return false;
        }
        else if (find_command(chat, "vendingexploit")) {
        gt::send_log("`2Succes Collected World Lock");
        ...
        return false;
            ...
            gt::send_log(
                "/proxy (Show All Proxy Command),  /freeblast [Target World] /spammsg [Nick Target] ( Spamming Message ) /spamtrade [Nick Target] ( Spamming Trade ) /setlevel (Setting World Level) /bulletin [ID BOARD] ( Spam Bulletin Board ) /dupebgl /tradeall (Trade All Player In World) /gsmexploit [MSG] (Global System Message Exploit ) /spamfriend (Spam Friend Request) /public ( Public World ) /unpublic ( Unpublic World ) /vps, /shutdown /country [ID,RU] ( Change Flag Country ) /folder, /sp, /spamwarp ( Spaming Join World ) /delworld, /spamsb, /closeexe, /blast, /changenews, /serverinfo /warp [Name World] (Warp Exploit)");
            return true;
        } 
        return false;
    }
...
bool events::in::variantlist(gameupdatepacket_t* packet) {
    ...
    switch (hs::hash32(func.c_str())) {
        ...
        case fnv32("OnConsoleMessage"): {
            varlist[1] = "`4[`wGaruda Suspend Commision`4]`` " + varlist[1].get_string();
...
bool events::in::gamemessage(std::string packet) {
    ...
    if (packet.find("msg|Updating Items...") != std::string::npos) { //skip updating items
        g_server->send(false, "action|enter_game\n", 3);
    }
